# frxiaobei-skills

[English](README.md) | 中文

适用于 Claude Code 和 OpenClaw 的 AI Skills

由 **[Elyfinn](https://elyfinn.com)** 出品 — 人机合伙人

---

## 快速开始

### Claude Code

```bash
# 注册 marketplace
/plugin marketplace add frxiaobei/frxiaobei-skills

# 安装 skill
/plugin install youyou-health@frxiaobei-skills
```

### OpenClaw

```bash
# 安装语音笔记
clawhub install elyfinn-voice-notes

# 安装健康管家
clawhub install youyou-health

# 安装图片下载
clawhub install article-image-downloader
```

---

## 可用 Skills

| Skill | 描述 | 文档 |
|-------|------|------|
| [elyfinn-voice-notes](skills/elyfinn-voice-notes) | 🎙️ 智能语音笔记 - 自动分类录音并生成结构化笔记 | [文档](skills/elyfinn-voice-notes/README.zh.md) |
| [youyou-health](skills/youyou-health) | 🩺 佑佑 - 私人健康管家，自然语言交互 | [文档](https://elyfinn.com/skills/youyou-health) |
| [article-image-downloader](skills/article-image-downloader) | 📷 文章图片下载 - 批量下载网页文章中的图片 | [文档](skills/article-image-downloader/SKILL.md) |

---

## 关于我们

我们是 **Elyfinn** — 一个人机合伙的小团队，探索人与 AI 协作的可能性。

- **@frxiaobei**（北哥）— 人类合伙人
- **Finn** 🦊（凡凡）— AI 合伙人

这些 skills 是我们自己用的工具，觉得有用就分享出来。务实、好用、解决实际问题。

---

## 致谢

站在巨人的肩膀上。特别感谢：

| 项目 | 用于 | 贡献 |
|------|------|------|
| [WellAlly-health](https://github.com/huifer/WellAlly-health) | youyou | 健康数据管理模式、17 个专科模板、命令结构设计 |

如果觉得这些 skills 有用，也请给原项目点个 ⭐。开源社区靠的是互相支持。❤️

---

## 贡献

发现 bug？有新想法？

- [提 Issue](https://github.com/frxiaobei/frxiaobei-skills/issues)
- 或者在 [X @frxiaobei](https://x.com/frxiaobei) 找我们

---

## 许可

MIT — 随便用，注明出处更好。

---

**frxiaobei@elyfinn** • [elyfinn.com](https://elyfinn.com) • [@frxiaobei](https://x.com/frxiaobei)
